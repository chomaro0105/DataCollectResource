package org.example.product.kafka.consumer;
...
/**
 * Async 의 경우에는 데이터를 모두 읽어들이고 난뒤 commit을 해야 하지만, 데이터를 읽어오는 과정에서 여러개의 프로세스가 실행되어
 * 각각 데이터를 읽어오지만, 한개만 읽어오는데 성공하고 나머지는 실패하더라도 성공했다고 판단해서 읽기 실패한 프로세스에 관련해서는 재요청을 하지 않는다.
 */
public class ConsumerAsync {
    public static void main(String[] args) {
        ...
        props.put("group.id", "gon-consumer01"); //컨슈머 그룹 아이디 정의합니다.
        ...
        consumer.subscribe(Arrays.asList("gon-basic01")); //구독할 토픽을 지정합니다.
